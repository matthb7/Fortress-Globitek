<?php
  ...
  function insert_user($user) {
    ...
    $sql .= "'" . $created_at . "',";
    $sql .= "'" . password_hash($user['password'], PASSWORD_BCRYPT) . "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  function update_user($user) {
    ...
    $sql .= "hashed_password='" . password_hash($user['password'], PASSWORD_BCRYPT) . "' ";
  ...
  function find_secrets_by_key($key) {
    global $db;
    $sql = "SELECT * FROM secrets ";
    $sql .= "WHERE id = '" . $key . "';";
    $secrets_result = db_query($db, $sql);
